...
## OBJECTIVE
Design and implement a multi-tenant Catalog-to-Offer EAV (Entity-Attribute-Value) relational database and API schema. The engine must decouple platform-level canonical catalog products from seller-level offers while dynamically inheriting category attributes, supporting seller-driven variant-versus-spec splitting, and driving sellable SKU variant matrix generation.
...
* **Attribute-Value Linkage Rule:** Attributes are linked to their allowed selectable values via `attribute_attribute_values`.
* **Category Mapping Rule:** Dynamic attribute groups are mapped directly to Categories (`category_attribute_groups`), NOT to catalog products.
...
* Catalog products contain ZERO direct dynamic attribute references. Dynamic attributes are resolved dynamically via:
  $$\text{Catalog Product} \longrightarrow \text{Category} \longrightarrow \text{Attribute Groups} \longrightarrow \text{Attributes} \longrightarrow \text{Attribute Values}$$

### 3. Seller Listing & Config Engine
* When a seller selects a `catalog_product` to list, a `seller_product` instance is initialized.
* The system fetches all attributes and allowed values associated with the product's `category_id`.
...
6. `attribute_values`
7. `attribute_attribute_values` (Attribute to Attribute Values mapping)
8. `catalog_products`
9. `seller_products`
10. `seller_product_attribute_configs` (`is_variant_driver` boolean toggle per seller product)
11. `seller_product_specs` (Static non-variant attribute value choices)
12. `seller_product_variants` (Sellable variant SKUs with price/stock/inventory)
13. `seller_variant_attribute_values` (Variant matrix value combination mappings)

---

## OUTPUT EXPECTATIONS
* Provide normalized SQL DDL scripts with Foreign Keys, Primary Keys, and Composite Indexing strategies.
* Provide JSON API payloads representing the state transitions:
  1. Initial category dynamic attribute fetch for a selected catalog product.
  2. Seller configuration payload submitting `is_variant_driver` flags and static choices.
