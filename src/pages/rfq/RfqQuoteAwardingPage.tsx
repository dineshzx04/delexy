...
export const RfqQuoteAwardingPage: React.FC = () => {
    ...
    () => [
      ...
      },
      {
        title: (
          ...
          >
            {rfq?.rfq_number ?? "RFQ Workspace"}
          </a>
        ),
      },
      { title: <span className="text-slate-800 font-semibold">Quote Award Revision</span> },
    ],
  ...
  const requesterName = useMemo(() => {
    if (!rfq) return "Buyer Account";
    const requesterParty = parties.find(p => p.id === rfq.requester_party_id || p.id === rfq.requester_id);
    return requesterParty?.display_name || rfq.requester_name || "Buyer Account";
  }, [rfq, parties]);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-16">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight m-0">RFQ Quotes Awarding</h1>
          <p className="text-slate-500 text-sm mt-1 m-0">
            Item-Seller Award Revision & Allocation: Evaluate proposals, negotiate item-wise seller allocations with revision rounds, and dispatch awards.
          </p>
        </div>
        <Button
      ...
      <Card size="small" className="border-slate-200/80 shadow-xs rounded-xl bg-white">
        ...
        >
          <Descriptions.Item label="RFQ Number">
            <span className="font-mono font-bold text-indigo-700">{rfq.rfq_number || "RFQ-000"}</span>
          </Descriptions.Item>
          <Descriptions.Item label="Buyer Account">
            <span className="font-medium text-slate-800">{requesterName}</span>
          </Descriptions.Item>
          <Descriptions.Item label="Sourcing Title">
            <span className="font-semibold text-slate-800">{rfq.title || "Contract Sourcing"}</span>
          </Descriptions.Item>
          <Descriptions.Item label="Submission Deadline">
            <span className="font-mono text-slate-700">{rfq.submission_deadline || "Standard Schedule"}</span>
          </Descriptions.Item>
          <Descriptions.Item label="Sourcing Status">
            <AntTag color="blue" className="font-semibold m-0">
              {rfq.status || "COLLECTING_QUOTES"}
            </AntTag>
          </Descriptions.Item>
          <Descriptions.Item label="Total Line Items">
            <span className="font-medium text-slate-800">{rfqItems.length} Requested Item(s)</span>
          </Descriptions.Item>
        </Descriptions>
      </Card>

      {/* Tier 3: Line Items Workspace */}
        ...
        {itemsComputedData.map(itemData => {
                  ...
                  if (sellerAward.award_status === "SELLER_REVISED" || variantAward.variant_award_status === "SELLER_REVISED") {
                    return (
                      <div className="flex flex-col items-center gap-1">
                        ...
                        >
                          <span className={isSelected ? "text-amber-600 font-bold" : "text-slate-500"}>
                ...
                return (
                  <div className="flex items-center justify-center">
                    <Checkbox
                      ...
                      onChange={e => handleToggleVariantSelection(item.id, variant, variant.sellerPartyId, variant.quoteId, e.target.checked)}
                    >
                      <span className={isSelected ? "text-indigo-600 font-bold text-xs" : "text-slate-500 text-xs"}>
          ...
          return (
            ...
            >
              ...
              <div className="bg-slate-50/90 px-4 py-3 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3 -mx-3 -mt-3 mb-4 rounded-t-xl">
                <div className="flex items-center gap-3">
                  <span className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-indigo-600 text-white font-bold text-xs">
                    {itemIndex}
                  </span>
                  <div>
                          ...
                          <tr className="bg-slate-50/90 text-slate-700 border-b border-slate-200">
                            ...
                            {sellerProposals.map(seller => (
                              <th
                                ...
                                className="p-2 border-r border-slate-200 text-center bg-indigo-50/30"
                              >
                              ...
                              <div className="flex items-center gap-3 text-xs">
                                ...
                                <div className="ml-2">
                                  {sellerQuoteAlloc.isConfirmed ? (
                                    <AntTag color="emerald" className="font-semibold text-xs m-0">
                                      <CheckCircleOutlined /> Confirmed ✓
                                    </AntTag>
                                    ...
                                    <Button
                                      ...
                                      className="bg-amber-600 hover:bg-amber-700 text-white font-semibold text-xs border-0 shadow-xs !h-7"
                                    >
                                    ...
                                    <Button
                                      ...
                                      onClick={() => handleOpenAwardRevisionModal(item, sellerQuoteAlloc)}
                                      className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs border-0 shadow-xs !h-7"
                                    >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {isFullyAllocated ? (
                        ...
                        <AntTag color="red" className="font-semibold text-xs m-0">
                          Over Allocated (+{allocatedQty - reqQty})
                        </AntTag>
                      ) : (
      ...
      {/* Tier 4: Award Revision Dispatch Modal */}
      <Modal
        ...
        okButtonProps={{ className: "bg-indigo-600 hover:bg-indigo-700 text-white font-medium border-0 shadow-xs" }}
      >
