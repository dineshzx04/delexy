...
import { catalogDb, type Attribute } from '../../data/catalog';
...
const Attributes: React.FC = () => {
  ...
  const GLOBAL_VALUES = useLiveQuery(() => catalogDb.attributeValues.toArray()) || [];
      ...
      render: (_: any, record: Attribute) => (
        <div className="flex items-center gap-2">
          ...
          <AntButton type="text" danger size="small" onClick={async () => {
            await catalogDb.attributes.delete(record.id);
          }}>Delete</AntButton>
        </div>
      ),
    },
  ...
  const handleSave = async (formValues: any) => {
    if (formValues.id) {
      await catalogDb.attributes.update(formValues.id, formValues);
    } else {
      ...
      await catalogDb.attributes.add(newAttr);
    }
  ...
  const handleSaveMapping = async (selectedRowKeys: React.Key[]) => {
    ...
    await catalogDb.attributes.update(currentMappingAttr.id, { valueIds: newValIds });
