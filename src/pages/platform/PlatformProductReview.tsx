...
const generateMockSubmissions = (): SubmittedProduct[] => {
  return [
    { 
      id: 'sub-1', submitterName: 'Acme Corp (Business)', productName: 'Ultra Widget 5000', categoryName: 'Industrial Sensors', status: 'Pending Review', submittedAt: '2023-10-26 14:30',
      payload: {
        basicInfo: { name: 'Ultra Widget 5000', description: 'Heavy duty sensor.', brand: 'Acme', partNumber: 'UW-5000', modelNumber: 'M-12', manufacturer: 'Acme Mfg' },
        attributes: [ { name: 'Voltage', value: '24V' }, { name: 'Material', value: 'Steel' } ],
        variants: [ { id: 'v1', name: 'Standard', price: 100, stock: 50, minOrder: 10 }, { id: 'v2', name: 'Pro', price: 200, stock: 10, minOrder: 5 } ]
      }
    },
    { 
...
const PlatformProductReview: React.FC = () => {
  ...
  const handleSubmitReview = (finalStatus: 'Approved' | 'Changes Requested') => {
  ...
  const getStatusTag = (status: string) => {
    switch (status) {
      case 'Approved': return <AntTag color="success">Approved</AntTag>;
  ...
  const columns = [
    ...
    { title: 'Status', dataIndex: 'status', key: 'status', render: (status: string) => getStatusTag(status) },
    {
      title: 'Action',
      key: 'action',
