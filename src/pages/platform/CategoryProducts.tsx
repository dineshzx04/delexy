...
import { catalogDb, type CatalogProduct } from '../../data/catalog';
...
const CategoryProducts: React.FC = () => {
  ...
  const breadcrumbs = useMemo(() => [
    ...
    { title: <span className="text-gray-900 font-semibold">Global Products</span> }
  ], []);
  ...
  const handleSave = async (values: any) => {
    ...
    if (editingProduct) {
      await catalogDb.products.update(editingProduct.id, values);
      ...
      await catalogDb.products.add(newProduct);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: CatalogProduct) => {
        const pathNames = record.categoryId ? getCategoryPathNames(record.categoryId) : [];
  ...
  return (
    <div className="w-full max-w-7xl pb-12">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 mb-1">Global Products</h1>
